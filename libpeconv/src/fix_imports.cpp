...
bool peconv::fix_imports(PVOID modulePtr, size_t moduleSize, const peconv::ExportsMapper& exportsMap, std::set<ULONGLONG> &not_recovered)
{
    ...
    while (parsedSize < maxSize) {
        ...
        if (!impUneraser.uneraseDllImports(lib_desc, dllCoverage, not_recovered)) {
            return false;
        }
        if (is_lib_erased) {
